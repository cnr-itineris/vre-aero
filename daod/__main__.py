...
#
data_path = './daod/data/'
graph_path = './daod/graphs/'
obs_list = './daod/aeronet.json'
#
aodl = 1
avg = 10

#obs_site = 'IMAA_Potenza'
#obs_date_min = '2025-05-01'
#obs_date_max = '2025-05-01'
hour_min = '00:00:00'
hour_max = '23:59:00'
...
def make_plot(aods, obs_date_min, obs_date_max, image_file):
	...
	plt.gca().xaxis.set_major_locator(mdates.DayLocator())  # Set major ticks to be daily

	# Calc locator & format interval
	#hr_loc = 12 if hr_diff > 24 else 1
	#hr_fmt = '%H' if hr_diff > 24 else '%H:%m'

	# Set minor ticks to show every 12 hours
	#plt.gca().xaxis.set_minor_locator(mdates.HourLocator(interval=hr_loc))  # Set minor ticks to be every 12 hours
...
def main(obs_site, obs_date_min, obs_date_max):
	...
	if date_diff > timedelta(days=30):
		print('ERROR: Date interval cannot be greater than 30 days')
		sys.exit(1)
...
def parse_args():
	...
	"""
	Parses command-line arguments for Daily Aerosol Optical Depth

	Returns:
		argparse.Namespace: Parsed command-line arguments
	"""
	parser = argparse.ArgumentParser(description="""
	This utility plots AERONET daily AOD measurements at wavelengths 340, 380, 440, 500, 675, 870, 1012, 1064 nm.
	Arguments:\n
